...
class Node:
  ...
  def add_shortest_h(self, dest, dist):
    self.shortest_h[dest] = dist
  
  def add_shortest(self, dest, dist):
    self.shortest[dest] = dist

  def check_status(self, closed = False):
    self.checked = closed
...
class AstarParams:
  def __init__(self, parent=None, g=0, h=0, f=0):
    self.parent = parent
    self.g = g
    self.h = h
    self.f = f
  ...
  def set_params(self, parent=None, g=0, h=0, f=0):
    self.parent = parent
    self.g = g
    self.h = h
    self.f = f
...
if edges_amount < 1:
  # 最短距離リストを生成
  def calc_shortest_routes(nodes_amount):
    shortest_routes = [[float('inf') for i in range(nodes_amount)] for i in range(nodes_amount)]
    for s in range(nodes_amount):
      shortest_routes[s][s] = 0
      nodes_costs = shortest_routes[s]
      new = set()
      if s == 0:
        for v in range(s + 1, nodes_amount):
          new.add(v)
        for v in range(s + 1):
          nodes[v + 1].check_status(True)
          for dest, dist in nodes[v + 1].edges:
            if dest < s + 1:
              continue
            temp = nodes_costs[v] + dist
            if temp < nodes_costs[dest - 1]:
              nodes_costs[dest - 1] = temp
              shortest_routes[dest - 1][s] = temp
      else:
        for v in range(s + 1, nodes_amount):
          new.add(v)
          nodes[v + 1].check_status()
        for v in range(s + 1):
          for dest, dist in nodes[v + 1].edges:
            if dest < s + 1:
              continue
      ...
      while new:
        ...
        for v in new:
          if nodes_costs[v] < nodes_costs[current]:
            current = v
        new.remove(current)
        nodes[current + 1].check_status(True)
        for dest, dist in nodes[current + 1].edges:
          if nodes[dest].checked == True:
            continue
          temp = nodes_costs[current] + dist
          if temp < nodes_costs[dest - 1]:
            nodes_costs[dest - 1] = temp
            shortest_routes[dest - 1][s] = temp
      shortest_routes[s] = nodes_costs
    return shortest_routes
  
  shortest_routes = calc_shortest_routes(nodes_amount)
      ...
      for j in jobs_w:
        if j.completed == False and j.validity[1] < t:
          ...
          if j.v_job == w.v:
            ...
            start_time = t + shortest_routes[w.v - 1][j.v_job - 1]
            sum_time = shortest_routes[w.v - 1][j.v_job - 1] + working_time
  ...
  def calc_cost(s, g):
    try:
      ...
      while open_l:
        ...
        # 現在のノード = ゴールの場合の終了処理
        if current == g:
          nodes[s].add_shortest(g, astar_params[current].g)
        ...
        for dest, dist in nodes[current].edges:
          ...
          try:
            ...
            if astar_params[dest].closed:
              if dest_f < astar_params[dest].f:
                ...
                astar_params[dest].set_params(current, dest_g, dest_h, dest_f)
          ...
          # destが未知の場合
            ...
            dest_g = dest_f - dest_h
            astar_params[dest] = AstarParams(current, dest_g, dest_h, dest_f)
            open_l.add(dest)
    ...
    for w in workers_sorted:
      ...
      if job_list:
        for job_id in job_list:
          j = jobs[job_id]
          if w.v < j.v_job:
            arg_1, arg_2 = w.v, j.v_job
          else:
            arg_1, arg_2 = j.v_job, w.v
          cost = calc_cost(arg_1, arg_2)
          working_time = - (- j.n_task // w.l_max)
          if j.v_job == w.v:
            start_time = t
            sum_time = working_time
            ...
            start_time = t + cost
            sum_time = cost + working_time
          finished_time = t + sum_time
          if finished_time - 1 < t_max:
            reward = j.calc_reward(start_time, finished_time, w.l_max)
            efficiency = reward / sum_time
            if efficiency > to_do[1]:
              to_do = [j.job_id, efficiency, finished_time, cost, working_time]
      # 仕事があるとき
      if to_do[0]:
        # 移動時間
        for _ in range(to_do[3]):
