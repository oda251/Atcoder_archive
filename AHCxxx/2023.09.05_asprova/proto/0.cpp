...
int	main(void)
{
	...
	// sort(dist_list.begin(), dist_list.end(), [](const tuple<int, int, int> &a, const tuple<int, int, int> &b){ return get<2>(a) < get<2>(b); });
	// sort(sd_list.begin(), sd_list.end(), [](const tuple<int, int, int> &a, const tuple<int, int, int> &b){ return get<2>(a) < get<2>(b); });
	// vector<int>	high_score_crops = get_crops_in_term_thres(sd_list, 0, T, T * 0.2);
	// cout << min(dist_list.size(), high_score_crops.size()) << endl;
	// for (int i=0; i<high_score_crops.size() && i<dist_list.size(); i++)
	// 	cout << get<0>(sd_list[high_score_crops[i]]) << ' ' << get<0>(dist_list[i]) << ' ' << get<1>(dist_list[i]) << ' ' << '1' << endl;
	...
	{
		crops_in_term = get_crops_in_term(sd_list, i * term, (i + 1) * term);
		for (int j=0; j<crops_in_term.size() && j<dist_list.size(); j++)
			result.emplace_back(get<0>(sd_list[crops_in_term[j]]), get<0>(dist_list[j]), get<1>(dist_list[j]), i * term + 1);
	}
	cout_score(result, sd_map);
...
vector<int>	get_crops_in_term_thres(vector<tuple<int, int, int>> sd_list, int ini_term, int fin_term, int least_term)
{
	...
	for (int i=0; i<sd_list.size() && get<2>(sd_list[i]) < fin_term; i++)
	{
		if (get<1>(sd_list[i]) < ini_term || get<2>(sd_list[i]) - get<1>(sd_list[i]) < least_term) continue ;
		dest.emplace_back(i);
	}
	return (dest);
}
