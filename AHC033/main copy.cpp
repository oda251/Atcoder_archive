...
void sim(Data &data, char store_flag, char export_flag, char pick_flag) {
	while (data.turn < 250) {
		for (auto &cln: data.clanes) {
			cln.store_flag = store_flag;
			cln.export_flag = export_flag;
			cln.pick_flag = pick_flag;
			if (!cln.is_working) {
				label:
