...
t_square find_space(t_data &data, t_sq_arrangement &sq_arr, int index, int req)
{
	...
	for (sq.start.y=0; sq.start.y<data.W; sq.start.y++)
	for (sq.start.x=0; sq.start.x<data.W; sq.start.x+=unit)
	{
		for (int a=unit; a<=req; a+=unit) {
			int b = req % a == 0 ? req / a : req / a + 1;
			sq.end.x = sq.start.x + a;
			sq.end.y = sq.start.y + b; 
			if (sq.end.x < data.W && sq.end.y < data.W && !occupied(sq, sq_arr, index))
				return sq;
		}
	}
