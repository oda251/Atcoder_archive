...
def aver_swap_detail_neo(g1, g2):
	...
	if len(g1) < len(g2):
		g1, g2 = g2, g1
		switched = True
	NUM2 = max(ceil(len(g1) / len(g2)), ceil(N // D / 2))
	if NUM2 > len(g1):
		if len(g1) != 1:
			NUM2 = len(g1) - 1
	for _ in range(NUM2 + 1):
...
def init_large_D(Q, D, N):
	...
	c_output(g1 + g2)
	comment("init_large_D 1")
	for i in range(len(g1) // 2):
		g1[i], g2[-i - 1] = aver_move(g1[i], g2[-i - 1])
		if i == 0:
			g1[i], g2[-i - 1] = aver_swap_const(g1[i], g2[-i - 1], max(ceil(len(g1[i]) / len(g2[-i - 1])), ceil(N // D / 2)))
	for i in range(len(g2) // 2):
		g2[i], g1[-i - 1] = aver_move(g2[i], g1[-i - 1])
		if i == 0:
			g2[i], g1[-i - 1] = aver_swap_const(g2[i], g1[-i - 1], max(ceil(len(g2[i]) / len(g1[-i - 1])), ceil(N // D / 2)))
	c_output(g1 + g2)
	comment("init_large_D 2")
	return g1, g2
...
def init_mid_D(Q, D, N):
	if N // D <= 10:
		...
		if D <= 10:
			...
			comment("init set_mid before sort")
			c_output(g)
			g = sort_one(g)
			comment("init set_mid")
			c_output(g)
			for i in range(D // 2):
				g[i], g[-i - 1] = aver_move(g[i], g[-i - 1])
				if len(g[-i - 1]) < N // D / 2:
					g[i], g[-i - 1] = aver_swap_const(g[i], g[-i - 1], max(ceil(N // D / 2), ceil(len(g[i]) / len(g[-i - 1]))))
					g[i], g[-i - 1] = aver_move(g[i], g[-i - 1])
			comment("init aver_move")
			c_output(g)
		else:
...
def solve_large_D(Q, D, N):
	...
	shuffle_groups(g1)
	shuffle_groups(g2)
	for i in range(max(2, D // 8)):
		pre = g1[i] + g1[-i - 1]
		g1[i], g1[-i - 1] = aver_move(g1[i], g1[-i - 1])
		if pre == g1[i] + g1[-i - 1] or i == 0:
			# g1[i], g1[-i - 1] = aver_swap_const(g1[i], g1[-i - 1], max(ceil(N // D / 2), ceil(len(g1[i]) / len(g1[-i - 1]))))
			g1[i], g1[-i - 1] = aver_swap_detail_neo(g1[i], g1[-i - 1])
		comment("HERE")
		c_output(g1 + g2)
		pre = g2[i] + g2[-i - 1]
		g2[i], g2[-i - 1] = aver_move(g2[i], g2[-i - 1])
		if pre == g2[i] + g2[-i - 1] or i == 0:
			# g2[i], g2[-i - 1] = aver_swap_const(g2[i], g2[-i - 1], max(ceil(N // D / 2), ceil(len(g2[i]) / len(g2[-i - 1]))))
	...
	while Q:
		...
		shuffle_groups(g2)
		comment("sorted")
		for i in range(max(2, ceil(D / 8))):
			pre = g1[i] + g1[-i - 1]
			# g1[i], g1[-i - 1] = aver_move(g1[i], g1[-i - 1])
			g1[i], g1[-i - 1] = aver_swap_detail_neo(g1[i], g1[-i - 1])
			if not pre == g1[i] + g1[-i - 1]:
				g1[i], g1[-i - 1] = aver_move(g1[i], g1[-i - 1])
			pre = g2[i] + g2[-i - 1]
			# g2[i], g2[-i - 1] = aver_move(g2[i], g2[-i - 1])
			g2[i], g2[-i - 1] = aver_swap_detail_neo(g2[i], g2[-i - 1])
			if not pre == g2[i] + g2[-i - 1]:
				g2[i], g2[-i - 1] = aver_move(g2[i], g2[-i - 1])
			# pre = g1[i] + g2[-i - 1]
			# g1[i], g2[-i - 1] = aver_swap_detail_neo(g1[i], g2[-i - 1])
			# if not pre == g1[i] + g2[-i - 1]:
			# 	g1[i], g2[-i - 1] = aver_move(g1[i], g2[-i - 1])
			# pre = g2[i] + g1[-i - 1]
			# g2[i], g1[-i - 1] = aver_swap_detail_neo(g2[i], g1[-i - 1])
			# if not pre == g2[i] + g1[-i - 1]:
			# 	g2[i], g1[-i - 1] = aver_move(g2[i], g1[-i - 1])
...
def solve_mid_D(Q, D, N):
	...
	while Q:
		shuffle_groups(g)
		if index % 2 == 0 or N // D > 15:
			for i in range(D // 2):
				g[i], g[-i - 1] = aver_move(g[i], g[-i - 1])
		else:
			for i in range(max(D // 4, 1)):
				if i == 0:
					g[i], g[-i - 1] = aver_swap_detail_neo(g[i], g[-i - 1])
					g[i], g[-i - 1] = aver_move(g[i], g[-i - 1])
				else:
					g[i], g[-i - 1] = aver_swap_const(g[i], g[-i - 1], max(ceil(len(g[i]) / len(g[-i - 1])), ceil(N // D / 2)))
		g = sort_one(g)
		c_output(g)
		comment("looped")
