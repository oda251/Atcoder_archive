...
def solve_large_D:
	...
	while Q:
		...
		c_output(g1 + g2)
		for i in range(2):
			g1[i], g1[-i - 1] = aver_swap_detail_neo(g1[i], g2[-i - 1])
			g1[i], g1[-i - 1] = aver_move(g1[i], g2[-i - 1])
			g2[i], g2[-i - 1] = aver_swap_detail_neo(g2[i], g1[-i - 1])
			g2[i], g2[-i - 1] = aver_move(g2[i], g1[-i - 1])
