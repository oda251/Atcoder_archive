...
int	main(void)
{
	...
	for (int t=2; t<=T; t++)
	{
		...
		sd_list.erase(remove_if(sd_list.begin(), sd_list.end(), [t](const auto &tp){ return get<1>(tp) == t - 1; }), sd_list.end());
...
{
	vector<tuple<int, int, int, int>>	result;
	vector<tuple<int, int, int>>	candedate_crops = get_candidate_crops(sd_list, map_arr.size() * 5);
	int	max_d;
	int	crop_id;
	...
	for (auto &it : map_arr)
	{
		...
		crop_id = select_crop(candedate_crops, t, max_d, get<2>(it));
		if (crop_id)
		{
			result.emplace_back(crop_id, get<0>(it), get<1>(it), t);
			map_d[get<0>(it)][get<1>(it)] = sd_map.at(crop_id).second;
			sd_list.erase(remove_if(sd_list.begin(), sd_list.end(), [crop_id](auto &tp){ return get<0>(tp) == crop_id; }), sd_list.end());
		}
